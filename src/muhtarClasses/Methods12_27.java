package muhtarClasses;

import java.util.Arrays;

public class Methods12_27 {
    public static void main(String[] args) {
/**1. write a method that can convert km to miles
 Hint: 1 km = 0.612 miles
 2. write a method that can convert gallons to litters
 Hint: 1 G = 3.75 L
 3. write a method that can print out the array in Descending order
 4. write a method that accepts 3 parameters: 2 numbers and one operator,
 and prints out the calculation
 if operator is not between [-, +, *, /, %] output should be Invalid Operator
 Ex: calculate(10, 2, "*") ==> 20;
 calculate(20, 2, "~") ==> invalid operator
 5. write a method that can calculate grade
 if score is 100 ~ 90 ==> A
 if score is 89 ~ 80 ==> B
 if score is 79 ~ 70 ==> C
 if score is 69 ~ 60 ==> D
 if score is 0 ~ 59 ==> F
 otherwise ==> Invalid Score
 * */
        System.out.println(convertkmTomiles(1));
        System.out.println(convertLittersToGallon(100));
        int[] array={5,3,2,1};
        Ddecsending(array);
        System.out.println();
        calculate(5,8,"+");
        calculate2(5,8,'+');
    }
    //TASK1
    public static double convertkmTomiles(double km){

        double miles=km*0.612;
        return miles;
    }
    //TASK2
    public static double convertLittersToGallon(double gallon){

        double litters=gallon*3.75;
        return litters;
    }
    //TASK3
    public static void Ddecsending(int[]array){
        //user passed me [2,3,1,5]
        Arrays.sort(array);
        //for iteration I need for loop I cannot use for each loop for this task bc for each loop
        //doesn't give result with the index
        for (int i = array.length-1; i >=0 ; i--) {
            System.out.print(array[i]+" ");
        }
    }
    public static void calculate(int a, int b, String operator){
//boolean result =(!operator.equals("*")|| !operator.equals("+")|| !operator.equals("/") ||
  //              !operator.equals("%"));
    //    if(result){
      //      System.out.println("invalid operator");
        //    return;
        //}
        if(operator.equals("-")){
            System.out.println(a-b);
        }else if(operator.equals("+"))
            System.out.println(a+b);
            else if(operator.equals("/"))
            System.out.println(a/b);
            else if(operator.equals("%"))
            System.out.println(a%b);
          else
            System.out.println("invalid operator");
    }
    public static void calculate2(int a, int b, char operator){
        switch(operator){
            case '*':
                System.out.println(a-b);
                break;
            case '+' :
                System.out.println(a+b);
                break;
            case'/':
                System.out.println(a/b);
            case '%':
                System.out.println(a%b);
                break;
            default:
                System.out.println("invalid operator");
        }
    }
    }

